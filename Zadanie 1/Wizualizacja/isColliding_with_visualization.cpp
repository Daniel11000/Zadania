...
int main()
{
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) {
                for (int i = 0; i < 3; i++) {
                    float dx = t1.points[i].x - event.mouseButton.x;
                    float dy = t1.points[i].y - event.mouseButton.y;
                    if (dx*dx + dy*dy < 25) {
                        selected_point = i;
                        break;
                    }
            ...
            if (event.type == sf::Event::MouseMoved && selected_point != -1) {
                moving = true;
                t1.points[selected_point].x = event.mouseMove.x;
                t1.points[selected_point].y = event.mouseMove.y;
                }
                }

                // check for collision
    bool collision = isColliding(t1, t2);

    window.clear(sf::Color(198, 242, 237));

    // draw triangle 1
    sf::VertexArray triangle1(sf::Triangles, 3);
    for (int i = 0; i < 3; i++) {
        triangle1[i].position = sf::Vector2f(t1.points[i].x, t1.points[i].y);
        if (collision)
            triangle1[i].color = sf::Color(255, 51, 51);
        else
            triangle1[i].color = sf::Color(128, 128, 128);
    }
    ...
    for (int i = 0; i < 3; i++) {
        triangle2[i].position = sf::Vector2f(t2.points[i].x, t2.points[i].y);
        if (collision == true)
            triangle2[i].color = sf::Color(255, 128, 0);
        else
            triangle2[i].color = sf::Color::Black;
    }
    window.draw(triangle2);

    // draw points
    for (int i = 0; i < 3; i++) {
        point.setPosition(sf::Vector2f(t1.points[i].x - 5, t1.points[i].y - 5));
        if (selected_point == i)
